...
public class EmeraldRepair extends SpecialCraftingRecipe
{
    public EmeraldRepair(Identifier identifier)
    {
        super(identifier);
    }
    ...
    {
        return RecipeInit.EMERALD_REPAIR;
    }
    ...
    protected MatchResult matchResult(CraftingInventory inventory)
    {
        ...
        for (int i = 0; i < inventory.size(); i++)
        {
            ...
            if (!stack.isEmpty())
            {
                ...
                if(item instanceof ToolItem && ((ToolItem) item).getMaterial() == ItemInit.EMERALD_TOOL_MATERIAL)
                {
