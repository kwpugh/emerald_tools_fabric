...
public class AmethystRepair extends SpecialCraftingRecipe
{
    public AmethystRepair(Identifier identifier)
    {
        super(identifier);
    }
    ...
    public ItemStack craft(CraftingInventory inventory)
    {
        ...
        if (matchResult.matches())
        {
            ...
            if (materialItem == Items.AMETHYST_SHARD)
            {
                damageRepair = 60;
            }
    ...
    protected MatchResult matchResult(CraftingInventory inventory)
    {
        ...
        for (int i = 0; i < inventory.size(); i++)
        {
            ...
            if (!stack.isEmpty())
            {
                ...
                if(item instanceof ToolItem && ((ToolItem) item).getMaterial() == ItemInit.AMETHYST_TOOL_MATERIAL)
                {
