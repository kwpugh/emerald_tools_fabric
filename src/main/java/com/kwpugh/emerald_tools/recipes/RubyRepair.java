...
public class RubyRepair extends SpecialCraftingRecipe
{
    public RubyRepair(Identifier identifier)
    {
        super(identifier);
    }
    ...
    protected MatchResult matchResult(CraftingInventory inventory)
    {
        ...
        for (int i = 0; i < inventory.size(); i++)
        {
            ...
            if (!stack.isEmpty())
            {
                ...
                if(item instanceof ToolItem && ((ToolItem) item).getMaterial() == ItemInit.RUBY_TOOL_MATERIAL)
                {
